...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setBackendError(error.response?.data?.message || "No account found with that email.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const S = {
    ...
    input: (hasError) => ({
      width: "100%", background: hasError ? colors.errorBg : colors.lavSofter,
      border: `1.5px solid ${hasError ? colors.errorBorder : "rgba(191,182,206,0.55)"}`,
      borderRadius: 9, padding: "12px 14px", fontFamily: "'DM Sans', sans-serif",
      ...
      <div style={S.page}>
        <div style={S.left}>
          <div style={S.caption}>
            ...
            <h2 style={S.captionH2}>Reclaim your<br /><em style={{ fontStyle: "italic" }}>sanctuary</em></h2>
            <p style={S.captionP}>No worries — we'll get you back to the island in no time.</p>
          </div>
        ...
        <div style={S.right}>
          ...
          <div style={S.formWrap}>
            <h1 style={S.h1}>Forgot your<br /><em style={{ fontStyle: "italic" }}>password?</em></h1>
            <p style={S.subP}>Enter your email and we'll take you straight to reset it.</p>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div style={{ marginBottom: 20 }}>
                ...
                <input
                  ...
                  value={email}
                  onChange={(e) => { setEmail(e.target.value); setEmailError(""); setBackendError(""); }}
                  onBlur={() => { setTouched(true); setEmailError(validate(email)); }}
                  style={S.input(touched && !!emailError)}
                />
                {touched && emailError && <p style={S.errorMsg}>{emailError}</p>}
              </div>
