...
const BookingManagement = () => {
  ...
  const validateForm = useCallback(() => {
    ...
    } else if (!users.some((u) => u.email === trimmedEmail)) {
        ...
        <div className="booking-table-wrapper">
          ...
          {!loading && !error && (
            <>
                      ...
                      <td>
                        {editingStatus === booking._id ? (
                          <select
                            ...
                            onChange={(e) => handleStatusEdit(booking._id, e.target.value)}
                          >
                        ...
                        <button
                          className="edit-btn"
                          onClick={() => setEditingStatus(editingStatus === booking._id ? null : booking._id)}
                        >
                          Edit
                        </button>
                      </td>
              ...
              <button className="add-booking-btn" onClick={() => setShowForm(true)}>
                Add New Booking
              </button>
            </>
          )}
        ...
        {showForm && (
          <div className="booking-form-modal">
            <div className="booking-form">
              ...
              <label>
                Adults:
                <select name="adults" value={formData.adults} onChange={handleFormChange}>
                  {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
              </label>
              <label>
                Children:
                <select name="children" value={formData.children} onChange={handleFormChange}>
              ...
              {formErrors.selectedRoom && <p className="error">{formErrors.selectedRoom}</p>}
              {formErrors.adults && <p className="error">{formErrors.adults}</p>}
              {formErrors.children && <p className="error">{formErrors.children}</p>}
              {formErrors.checkIn && <p className="error">{formErrors.checkIn}</p>}
              {formErrors.checkOut && <p className="error">{formErrors.checkOut}</p>}
              {totalPrice > 0 && <p>Total Price: ${totalPrice.toFixed(2)}</p>}
              <button type="button" onClick={handleConfirmPay} disabled={!isFormValid()}>
                CONFIRM & PAY
              </button>
